...
```bash
boxrun shell IMAGE [--name NAME] [flags...]
```

Create a box and immediately attach to it — combines `create` + `attach` in one command.

```bash
boxrun shell ubuntu
boxrun shell python:slim --name pydev --memory 1024
```
...
boxrun run python:slim --timeout 30 -- python3 -c "print(42)"
```
