...
<Note>
**Apple Silicon + Node.js:** the darwin-arm64 native package is published only up to v0.9.7, so `npm install @boxlite-ai/boxlite@latest` installs a version that fails to load. Pin `@boxlite-ai/boxlite@0.9.7` on Apple Silicon.
</Note>
...
| `npm install` succeeds but the module fails to load on a Mac | The darwin-arm64 native package stops at v0.9.7 while the JS package is newer | Pin `@boxlite-ai/boxlite@0.9.7` |
