...
<Tabs>
  <Tab title="Python">
    ...
    try:
        ...
        await box.shutdown()  # Python SDK uses shutdown()
    ```
  </Tab>
  <Tab title="Node.js">
    ```typescript
    ...
    try {
      ...
      await box.stop();  // Node.js SDK uses stop()
    }
    ```
  </Tab>
</Tabs>
...
<Note>
The Python SDK uses `shutdown()` for explicit cleanup. The Node.js SDK uses `stop()`. Both do the same thing — stop the VM and release resources.
</Note>
