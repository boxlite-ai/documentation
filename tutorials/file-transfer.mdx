  ...
  <Tab title="Node.js">
    ...
    async function main() {
      ...
      try {
        ...
        console.log(outputResult.stdout);
      } finally {
        await box.stop();
        await runtime.remove(box.id);
      }
  ...
  <Tab title="Node.js">
    ...
    async function main() {
      const box = new SimpleBox({
        image: 'python:slim',
        volumes: [
          { hostPath: '/host/datasets', guestPath: '/mnt/data', mode: 'ro' },
          { hostPath: '/host/results', guestPath: '/mnt/results', mode: 'rw' },
        ],
      });
...
**When to use:** Shared datasets or configuration that multiple boxes need access to. Use `"ro"` (read-only) for input data to prevent accidental modification.

<Warning>
  Volume mounts give the guest direct access to host files. Always use `"ro"` mode for input data. Only use `"rw"` for designated output directories.
</Warning>
...
async def main():
    ...
    try:
        ...
        # 3. Run the analysis
        execution = await box.exec("python", ["/workspace/analyze.py"])
        result = await execution.wait()
...
<CardGroup cols={2}>
  ...
  <Card title="AI agent integration" icon="gear" href="/guides/ai-agent-integration">
    Advanced file transfer patterns, security boundaries, and read-only volume mounts.
  </Card>
</CardGroup>
