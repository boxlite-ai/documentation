...
| Fork or branch sandbox state | [**SimpleBox**](/getting-started/core-concepts#box-types) or [**CodeBox**](/getting-started/core-concepts#box-types) | [Snapshot, fork, restore](/tutorials/snapshot-fork-restore) *(coming soon)* |
...
<CardGroup cols={2}>
  ...
  <Card
    title="Snapshot, fork, restore (coming soon)"
    icon="code-branch"
    href="/tutorials/snapshot-fork-restore"
  >
    Take a snapshot, branch two execution paths from the same state, then roll one back — like git branch for sandbox environments.
  </Card>
  <Card
...
Once you've worked through these tutorials, dive into the [How-to guides](/guides/index) for production patterns like concurrency, timeout handling, security configuration, and deployment.
