...
`working_dir` (or `exec(cwd=...)`) must be a **directory that already exists in the image**, and the two fail at different moments.

With `working_dir="/work"` on `alpine`, the constructor returns fine but **entering `async with` raises** — there is never a box to exec on:

```text
RuntimeError: internal error: Container init failed: Failed to create container: ...
error in executing process : failed to unix syscall
```

With `exec(cwd="/work")` the box starts normally and only that call fails, leaving the box usable:
