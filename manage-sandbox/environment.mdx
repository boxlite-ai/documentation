...
> Important: `entrypoint + cmd` runs as the container init, so **the box stops when it exits**. Keep it long-running if you want to `exec` afterwards, as the example below does. See [What stops a box](/manage-sandbox/lifecycle#what-stops-a-box) for the rules that follow from this.
