...
Read the current state with `box.info().state.status`. **`info()` is async** — `await` it. Field names are exact and easy to get wrong: see [`BoxInfo` / `BoxStateInfo`](#boxinfo-boxstateinfo-state-access).
...
### Forgetting to await `Box.info()`

`info()` is an **async** method.
